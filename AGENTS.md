...
Перед любой сборкой, генерацией проекта, запуском линтеров или тестов агент **обязан прочитать `BUILD.md`** и следовать актуальным инструкциям из него.
...
- **Никаких optimistic update для подтверждаемого состояния**: playback state, текущая позиция трека, `isPlaying`, активный item, buffering/error state и другие данные, зависящие от движка/сети/хранилища, обновляются в UI только после подтверждения из нижележащего слоя (`player`, `repository`, `controller`). Допустим только краткоживущий локальный preview во время прямого пользовательского gesture, например drag seek; после завершения gesture UI обязан вернуться к подтверждённому состоянию и не выдавать предположение за факт.
...
- **Отступы (spacing)**: предпочитать **spacing**-токены из дизайн-системы кастомным локальным переменным/константам и хардкоду чисел (`dp`/`sp` на Android, голые значения в SwiftUI на iOS). Если для сценария есть токен — использовать его (`MultiplayerTheme.spacing` в `android/core/ui` и `CoreUI` на iOS), а не дублировать величину в своём коде.
